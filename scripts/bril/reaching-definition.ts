...
import {
  type RDResult,
  rdWithMachine,
} from '../../pages/posts/2024/fantastic-cfgs/utils/rd'
import {
  addIdToHast,
...
export function reachingDefinitionTransformer(
  ...
  return {
    ...
    preprocess(this, code) {
      ...
      if (this.options.lang !== 'bril' || !match) return
    ...
    span(this, hast, row, col) {
      if (this.meta.transformRd === undefined) return
      ...
      if (explanation && explanation.length > 0) {
        ...
        const isVariable =
          scopes.filter(s => s.scopeName === VARIABLE_SCOPE).length > 0
        const isDestVariable =
          scopes.filter(s => s.scopeName === DEST_VARIABLE_SCOPE).length > 0
        ...
        if (isDestVariable) {
          ...
          for (const rdInfo of this.meta.rdInfo.values()) {
            if (rdInfo.get(row) === undefined) continue
